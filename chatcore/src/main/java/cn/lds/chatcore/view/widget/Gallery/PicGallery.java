package cn.lds.chatcore.view.widget.Gallery;
...
import cn.lds.chatcore.view.widget.ImageView.MyImageView;
...
public class PicGallery extends Gallery {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (view instanceof MyImageView) {
            ...
            float min_distance = MyApplication.getInstance().getWidth() / 4f;
            ...
            if ((int) width <= MyApplication.getInstance().getWidth() && (int) height <= MyApplication.getInstance().getHeight())// 如果图片当前大小<屏幕大小，直接处理滑屏事件
            {
                ...
                if (distanceX > 0)// 向左滑动
                {
                    if (r.left > 0 || right < MyApplication.getInstance().getWidth()) {// 判断当前ImageView是否显示完全
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                ...
                {
                    if (r.right < MyApplication.getInstance().getWidth() || left > 0) {
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                        imageView.postTranslate(-distanceX, -distanceY);
                    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                if (view instanceof MyImageView) {
                    ...
                    // + height + ",screenWidth="
                    // + MainActivity.screenWidth + ",screenHeight="
                    // + MainActivity.screenHeight);
                    if ((int) width <= MyApplication.getInstance().getWidth() && (int) height <= MyApplication.getInstance().getHeight())// 如果图片当前大小<屏幕大小，判断边界
                    {
                        break;
                    }
                    float v[] = new float[9];
                    ...
                    if (top < 0 && bottom < MyApplication.getInstance().getHeight()) {
                        // imageView.postTranslateDur(-top, 200f);
                        imageView.postTranslateDur(MyApplication.getInstance().getHeight() - bottom, 200f);
                    }
                    if (top > 0 && bottom > MyApplication.getInstance().getHeight()) {
                    ...
                    if (left < 0 && right < MyApplication.getInstance().getWidth()) {
                        // imageView.postTranslateXDur(-left, 200f);
                        imageView.postTranslateXDur(MyApplication.getInstance().getWidth() - right, 200f);
                    }
                    if (left > 0 && right > MyApplication.getInstance().getWidth()) {
