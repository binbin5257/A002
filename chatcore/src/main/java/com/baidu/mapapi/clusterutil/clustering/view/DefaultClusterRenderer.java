...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    private LayerDrawable makeClusterBackground() {
        ...
        int strokeWidth = (int) (mDensity * 0);
//        int strokeWidth = (int) (mDensity * 3);
    ...
    private int getColor(int clusterSize) {
        ...
        int k = Color.HSVToColor(new float[]{
                95, 81, 82
        });
        return k;
...
//        mColoredCircleBackground.getPaint().setColor(getColor(bucket));//圈的颜色
        mColoredCircleBackground.getPaint().setColor(MyApplication.getInstance().
                getApplicationContext().getResources().getColor(R.color.locationmarker_color));//圈的颜色
//        BitmapDescriptor descriptor = mIcons.get(bucket);  //重新计算
        BitmapDescriptor descriptor = null;
        if (descriptor == null) {
            int i = 0;
            int k = 0;
            for (T item : cluster.getItems()) {
                k = Integer.parseInt(((MyCluterItem) item).getmDataBean().getAvailableVehicle());
                i = i + k;
            }
            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(i + ""));//圈内数字
