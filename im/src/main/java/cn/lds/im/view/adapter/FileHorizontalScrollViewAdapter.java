...
public class FileHorizontalScrollViewAdapter extends BaseAdapter {

    private Context mContext;
    private LayoutInflater mInflater;
    private List<FileModel> horizontalList;

    public FileHorizontalScrollViewAdapter(Context context, List<FileModel> horizontalList) {
...
//        OrganizationTable mDatas = (OrganizationTable) getItem(position);
        if (horizontalList != null && horizontalList.size() > 0) {
            String data = horizontalList.get(position).getName();
