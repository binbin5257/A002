...
public class ExpireCouponFragment extends Fragment {
    ...
    private void initData() {
        ModuleHttpApi.getCouponList(page,"OUTDATED");
    }
    public void onEventMainThread(HttpRequestEvent event) {
        HttpResult httpResult = event.httpResult;
        String apiNo = httpResult.getApiNo();
        if (!(ModuleHttpApiKey.getCouponList + "OUTDATED").equals(apiNo)) {
            return;
        }
        String result = httpResult.getResult();
        if (!ToolsHelper.isNull(result)) {
            switch (apiNo) {
                case ModuleHttpApiKey.getCouponList + "OUTDATED":
