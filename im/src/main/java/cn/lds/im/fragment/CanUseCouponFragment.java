...
//        LoadingDialog.showDialog(getActivity(), getString(R.string.search_footer_zhengaijiazai));
        ModuleHttpApi.getCouponList(page,"RELEASE");
    }
    public void onEventMainThread(HttpRequestEvent event) {
        HttpResult httpResult = event.httpResult;
        String apiNo = httpResult.getApiNo();
        if (!(ModuleHttpApiKey.getCouponList + "RELEASE").equals(apiNo)) {
            return;
        }
//        LoadingDialog.dismissDialog();
        ...
        if (!ToolsHelper.isNull(result)) {
            switch (apiNo) {
                case ModuleHttpApiKey.getCouponList + "RELEASE":
